...
case class Prop(run: (MaxSize, TestCases, RNG) => Result) {
  def &&(p: Prop) = Prop {
    (max, n, rng) =>
      run(max, n, rng) match {
        case Right((_, n)) => p.run(max, n, rng).right.map { case (s, m) => (s, n + m) }
        case l => l
      }
  }
...
object Prop {
  ...
  def forAll[A](g: Int => Gen[A])(f: A => Boolean): Prop = Prop {
    (max, n, rng) =>
      ...
      val p: Prop = props.map(p => Prop((max, _, rng) => p.run(max, casesPerSize, rng))).
        reduceLeft(_ && _)
  ...
  val pint: Gen[Par[MaxSize]] = Gen.choose(0, 10) map (Par.unit(_))
  val p4: Prop =
...
object Gen {
  ...
  */
  ...
  def interleave[A](s1: Stream[A], s2: Stream[A]): Stream[A] =
    s1.zipAll(s2).flatMap { case (a, a2) => //noinspection ScalaUnnecessaryParentheses
      Stream((a.toList ++ a2.toList): _*)
    }
