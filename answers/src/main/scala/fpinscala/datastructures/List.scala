...
object List {
  ...
  /** 练习 3.1
    * 看代码猜结果，锻炼模式匹配脑回路。
    */
  val x1: Int = List(1, 2, 3, 4, 5) match {
  ...
  */
  ...
  /** 练习 3.3
    ...
    * 强烈建议（同上3.2）： 对空列表进行setHead的时候，抛出异常。
    * 实现setHead.
    */
  ...
  /** 练习 3.6
    ...
    * 实现init。
    * 第一回合： 非尾递归，对大列表，会有stackoverflow。
    */
  ...
  /** 练习 3.7
    ...
    * 用foldRight来重新实现函数product，当碰到0.0时，中止递归，直接返回0.0，可以吗？
    * 不行，做不到。我们需要一个_non-strict_evaluation的foldRight版本，才可以实现提前中止运行。
    * 我们到第五章再来讨论这个话题。
    */
  ...
  /** 练习 3.8
    ...
    * 把Nil和Cons作为参数，调用foldRight玩玩看，会玩出来什么花样。
    */
  ...
  /** 练习 3.13 2/3 难
    ...
    * 用foldLeft和reverse来实现foldRight。
    */
  def foldRightViaFoldLeft[A, B](l: List[A], z: B)(f: (A, B) => B): B =
    foldLeft(reverse(l), z)((b, a) => f(a, b))

  /** 练习 3.13 3/3 难
    * 用foldLeft来实现foldRight。
    */
  ...
  /** 练习 3.15
    ...
    * 实现concat，运行时间和所有列表的总长度线性先关，最好利用之前已经实现的函数。
    */
  ...
  /** 练习 3.20
    ...
    * 写一个函数，就像map一样的修改每一个元素，不过这一次，那个就该函数`f`返回的是列表类型。
    */
  ...
  /** 练习 3.23
    ...
    * applies here. By putting the `f` in the second argument list, Scala can infer its type from the previous argumentlist.
    * 推广3.22，写一个通用的zipWith函数。
    */
